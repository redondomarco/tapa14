...
if [ "$1" = 'uwsgi' ]; then
  ...
  if [ "$WEB2PY_PASSWORD" != '' ]; then
    python3 -c "from gluon.main import save_password; save_password('$WEB2PY_PASSWORD',443)"
  fi
  # run uwsgi
...
if [ "$1" = 'http' ]; then
  ...
  if [ "$WEB2PY_ADMIN_SECURITY_BYPASS" = 'true' ]; then
    ...
    python3 -c "from gluon.main import save_password; save_password('$WEB2PY_PASSWORD',8080)"
...
if [ "$1" = 'https' ]; then
  ...
  python3 -c "from gluon.main import save_password; save_password('$WEB2PY_PASSWORD',8080)"
  # run uwsgi
  exec uwsgi --wsgi wsgihandler:application $UWSGI_OPTIONS --https 0.0.0.0:8080,/certs/web2py.crt,/certs/web2py.key
fi
